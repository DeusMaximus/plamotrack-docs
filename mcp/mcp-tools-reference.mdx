...
<Accordion title="get_meta">
  Returns the app version and the instance's reference currency — the currency an order's own `currency_code` defaults to when you omit it while logging an order.
</Accordion>
...
<Accordion title="create_order">
  Creates a full order with line items. Kit lines fan out into individual kit rows (two Zakus on one line becomes two separate kits in your collection). Retailers are matched by name or created automatically if they don't exist yet. Every item line carries its own `currency_code`; only the order-level `currency_code` may be omitted, in which case the instance's reference currency is used.
</Accordion>
