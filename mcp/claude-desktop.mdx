...
Claude Code talks to the endpoint directly, no bridge needed, and picks the current MCP protocol revision on its own. There is nothing else to configure.
...
<Tip>
  A client configured without a token is refused at its very first request, whichever protocol revision it speaks. If Claude reports that the server needs authentication, check that the `Authorization` header or the `env` value carries `Bearer ptk_…` with the space in between.
</Tip>
