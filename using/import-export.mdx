...
description: "Export your entire collection as a ZIP of CSVs, import it back to restore or merge, and use the starter sheet to bring in an existing spreadsheet with one row per kit."
---

Your collection data is always yours. Export it as a ZIP of plain CSVs, import it back at any time, or move it to a new instance. Every import shows you a full preview of what will change before anything is written — nothing is committed until you click **Apply**.
...
Every import — regardless of mode — shows you a full preview of what will change: records to add, update, or delete. Nothing is written until you click **Apply**.
...
Every exported row includes an internal ID. Importing an archive back into the same instance matches those IDs to your existing records — so re-importing is always a no-op. Nothing changes, because nothing has changed. Use this freely for backup verification.
