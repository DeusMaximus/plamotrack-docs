...
These steps are for an installation from the release files, as in [Installation](/installation). If you installed with `git clone`, see [If you installed with Git](#if-you-installed-with-git).

<Steps>
  <Step title="Back up">
    Follow [Backups](/configuration/backups): the database and your `.env`. A CSV export is a useful extra copy of your collection, but it does not include your owner account, sessions or access tokens.
  </Step>
  <Step title="Read the release notes">
    Read the notes below for every version newer than yours, and make any configuration changes they ask for before starting the new version.
  </Step>
  <Step title="Go to your plamotrack folder">
    ```bash
    cd ~/plamotrack
    ```
  </Step>
  <Step title="Download the new version's files">
    Use the number of the version you're updating to; the newest is **v0.5.2-alpha**. Run this as one line. It replaces the old release files, and your `.env` stays as it is:
  ...
  <Step title="Check the files">
    ...
    Every file should say **OK**. Don't start the new version from files that fail this check.
  </Step>
  <Step title="Start the new version">
    ...
    The `migrate` service applies any new database migrations before the API starts. You do not run a separate migration command.
  </Step>
...
**If the update fails**, read the migration output and [Troubleshooting](/configuration/troubleshooting) before continuing. Keep your database intact. To go back, download the previous version's files the same way and run the same `up`. That's safe when the release notes say the two versions share a database schema. Across a migration, use the previous version with your pre-update backup, following [the restore guide](/configuration/backups#restoring-a-dump). Some database downgrades discard data; exceptions are called out below.

## If you installed with Git

Versions before v0.5.2-alpha were installed with `git clone` and built on your computer. You can move that installation onto the release files and keep everything: your collection, your owner account and sign-ins, your access tokens and your connected AI assistants. These steps assume your folder is `~/plamotrack`, as the older Installation guide made it. Use your own folder's path if it's different.

<Steps>
  <Step title="Back up">
    Follow [Backups](/configuration/backups): the database and your `.env`.
  </Step>
  <Step title="Check for a file of your own">
    From your `plamotrack` folder, run:

    ```bash
    ls docker-compose.override.yml
    ```

    It should say there is no such file. If there is one, you or a guide you followed created it, and these steps don't cover it. Stop here and keep your installation as it is.
  </Step>
  <Step title="Stop plamotrack">
    ```bash
    docker compose down
    ```

    Never add `-v` here: that would delete your database.
  </Step>
  <Step title="Move the Git folder aside">
    ```bash
    cd ~
    mv plamotrack plamotrack-source
    ```
  </Step>
  <Step title="Download the release files into a new plamotrack folder">
    Follow steps 1 and 2 of [Install plamotrack](/installation#install-plamotrack): make `~/plamotrack`, download the files and check them. Don't create a new `.env` from `env.example`.
  </Step>
  <Step title="Bring your existing settings across">
    ```bash
    cp ~/plamotrack-source/.env ~/plamotrack/.env
    ```

    Use your existing `.env`, not the template. It holds the password your database was created with, and your sign-in and assistant settings. If it has `COMPOSE_FILE` or `COMPOSE_PATH_SEPARATOR` lines, delete them: the release files don't need them.
  </Step>
  <Step title="Start plamotrack">
    ```bash
    cd ~/plamotrack
    docker compose up -d --no-build --wait
    ```

    Sign in and check your collection. Your AI assistants stay connected.
  </Step>
</Steps>

**Why the folder's name matters:** Docker Compose uses the folder's name to name the volume that holds your database. The new folder has the same name, so it uses the same volume and finds the same database. The old folder, now `plamotrack-source`, would get a volume of its own: if you ran it, it would start a separate, empty plamotrack and couldn't touch yours. Once you're happy with the move, you can delete it.

**To go back:** run `docker compose down` in `~/plamotrack`. Then swap the folder names back with `cd ~ && mv plamotrack plamotrack-release && mv plamotrack-source plamotrack`, and run `docker compose up -d --build --wait` in the Git folder. Your database is the same one either way.

<Accordion title="Updating a Git installation instead">
  If you'd rather keep building from Git:

  <Steps>
    <Step title="Back up">
      Follow [Backups](/configuration/backups).
    </Step>
    <Step title="Add two lines to your .env">
      An installation set up from v0.5.0-alpha or earlier doesn't have them. Add them at the top of `.env`:
      ...
      Without them, the update stops with `error from registry: denied` before changing anything.
    </Step>
...
<Update label="v0.5.2-alpha" description="23 September 2026">
  ## Install with Docker alone

  - **Each release now carries its own install files.** The release page has a `docker-compose.yml` with every image pinned by digest, for both `linux/amd64` and `linux/arm64`, plus `env.example`, the Gunpla skill's `plamotrack-gunpla.zip`, `release.json` and `SHA256SUMS`. In an empty directory, check the files with `sha256sum -c SHA256SUMS`, copy `env.example` to `.env`, set the database password, and run `docker compose up -d --no-build --wait`. No Git checkout or registry login is needed.
  - **No moving channel.** There is no `latest` tag. Updating means choosing the next version on purpose.
  - The application itself is unchanged from 0.5.0. There is no 0.5.1: that version was tagged but never published, because GitHub renamed one of its files on upload.

  **Update notes:** From 0.5.0 there are no migrations, authentication changes or new settings. **A source installation needs two lines at the top of `.env` before it updates:**
  ...
  Without them, the update stops with `error from registry: denied` before changing anything. Better still, move it onto the release files: see [If you installed with Git](#if-you-installed-with-git). Returning to `v0.5.0-alpha` means checking out that tag and **removing those two lines** (0.5.0 has no `docker-compose.build.yml`), then rebuilding. Moving an existing installation onto the release files keeps its folder name, and with it the same database volume.

  **Known limitations at release:** On the stock steppers, keyboard focus is not kept on the button after a press ([#268](https://github.com/DeusMaximus/plamotrack/issues/268)). The order-dialog limitation from 0.4.0 is still open ([#238](https://github.com/DeusMaximus/plamotrack/issues/238)).

  [Full release notes](https://github.com/DeusMaximus/plamotrack/releases/tag/v0.5.2-alpha)
</Update>

<Update label="v0.5.0-alpha" description="20 September 2026">
  ## plamotrack on a phone and a tablet

  - **Three layouts, chosen by the width of the window.** On a phone: a tab bar under your thumb, lists as cards, forms as full-screen sheets with their buttons at the foot of the screen, and one **Filter and sort** sheet on Kits and Orders. On a tablet or in a narrow window: a rail of icons, and tables that fold their columns to fit instead of scrolling sideways. From 1280 px wide it is the desktop layout as before. See [Phone & Tablet](/using/phone-and-tablet).
  - **Add it to your home screen** on iOS, iPadOS or Android and it opens in its own window, still signed in. There is no offline mode.
  - **Settings on a phone** opens as a list of its sections. **Data management** there offers the exports only — importing and the blank templates need a screen at least 768 px wide.
  - **Large text holds up.** On a phone, rows wrap and long names break rather than pushing anything off the screen, up to two and a half times the browser's default text size.
  - Keyboard focus keeps its place when a tablet is turned or a window is resized, including with a dialog open.

  **Update notes:** From 0.4.1, there are no new migrations, authentication changes or settings. Run the normal update and sign in as before. Returning to `v0.4.1-alpha` only requires checking out that tag and rebuilding; the two versions use the same database schema.

  **One change on the desktop:** between 1280 and 1361 px wide, the Orders table now shows the order number under the retailer's name, where an ordinary row used to push the edit control off the table's edge. Nothing else from 1280 px up has moved.

  **Known limitations at release:** On the stock steppers, keyboard focus is not kept on the button after a press ([#268](https://github.com/DeusMaximus/plamotrack/issues/268)). The order-dialog limitation from 0.4.0 is still open ([#238](https://github.com/DeusMaximus/plamotrack/issues/238)).

  [Full release notes](https://github.com/DeusMaximus/plamotrack/releases/tag/v0.5.0-alpha)
</Update>

<Update label="v0.4.1-alpha" description="15 September 2026">
  ...
  **Update notes:** From 0.4.0, there are no new migrations, authentication changes or settings. Run the normal update. Connected AI clients stay connected and personal access tokens keep working. An instance updated from 0.3.x or 0.4.0 may hold a stored copy of Claude.ai's or ChatGPT's client description from before; it is used until the first fresh fetch and then removed — nothing to do. One thing only an unusual client notices: a dynamic registration that asks for the `private_key_jwt` authentication method is refused at registration; none of the tested clients asks for it. Returning to `v0.4.0-alpha` only requires checking out that tag and rebuilding; the two versions use the same database schema.
...
## Data checks for older instances

Two situations a much older instance can be in. Neither affects a fresh installation.

### Kit quantities from early imports

Importers before 0.2.7 could leave a kit order line holding a different number of kits than its quantity said. Nothing in the app minds, but an archive exported from such a collection is refused when you restore it with **Replace everything** ("this line says quantity N, but this upload supplies M kit(s)"). Check once, before your next export, from your `plamotrack` folder:
...
No rows means nothing to do. For each row it lists, open that order in the app and save the line: the order editor reconciles the count against the kits actually there — it spawns a missing one, or leaves an extra one in place once you set the quantity to match. Then export again.

### Databases from before the first public release

The migration that introduced pending and received orders marked every order that existed before it as received — those orders had their stock applied at entry, so they were received by definition — but did not touch the kits those orders had spawned. A database that crossed that migration before 6 August 2026 can therefore hold a received order whose kit still says **Ordered** or **In Transit**, which the app would never produce on its own. It is cosmetic, and deliberately not repaired by a later migration, since a repair now would overwrite kit statuses set by hand since. If you see such a kit, edit its status; that is the whole fix.
