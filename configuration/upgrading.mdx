...
3. **Pull the code:**
   ...
   git pull
   ```
4. **Check the top of your `.env` for a `COMPOSE_FILE` line.** An installation set up from `v0.5.0-alpha` or earlier doesn't have one. Add these two lines at the top of `.env`, exactly as written:
   ```ini
   COMPOSE_PATH_SEPARATOR=:
   COMPOSE_FILE=docker-compose.yml:docker-compose.build.yml
   ```
   They make every `docker compose` command in the folder build plamotrack from your checkout, as before. Without them, the next command looks for a published image instead and stops with `error from registry: denied`, before changing anything.
5. **Rebuild:**
   ```bash
...
<Update label="v0.5.2-alpha" description="23 September 2026">
  ...
  **Update notes:** From 0.5.0 there are no migrations, authentication changes or new settings. **A source installation needs two lines at the top of `.env` before it updates:**
  ...
  Without them, the update stops with `error from registry: denied` before changing anything. See step 4 of [How to update plamotrack](#how-to-update-plamotrack). Returning to `v0.5.0-alpha` means checking out that tag and **removing those two lines** (0.5.0 has no `docker-compose.build.yml`), then rebuilding. Keep updating a source installation from source. Moving an existing installation onto the release files isn't covered yet: in a new directory they would start a new, empty collection.
