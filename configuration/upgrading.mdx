...
2. **Read the release notes below** for every version newer than yours. Make any required configuration changes before starting the new version.
3. **Download the new version's files into your `plamotrack` folder**, replacing the old ones. Your `.env` stays as it is. Use the number of the version you're updating to; the newest is **v0.5.2-alpha**:
   ```bash
   cd ~/plamotrack
   ```
   ```bash
   ...
   ```
   Then check them with `sha256sum -c SHA256SUMS` (on macOS: `shasum -a 256 -c SHA256SUMS`). Every file should say **OK**.
...
**Why the folder's name matters:** Docker names your database after the folder. The new folder has the same name, so it finds the same database. The old folder, now `plamotrack-source`, would start a separate, empty plamotrack if you ran it, and can't touch yours. Once you're happy with the move, you can delete it.
...
<Accordion title="Updating a Git installation instead">
  If you'd rather keep building from Git, add these two lines at the top of your `.env` before updating. An installation set up from v0.5.0-alpha or earlier doesn't have them:
  ...
  Without them, the update stops with `error from registry: denied` before changing anything. Then back up, run `git pull`, and run `docker compose up -d --build --wait`. Keep `--build` on every update.
</Accordion>
