...
<Accordion title="error from registry: denied, after updating a Git installation">
  You installed with `git clone`, and your `.env` is older than the current Compose files: it has no `COMPOSE_FILE` line, so Docker Compose looked for a published image instead of building plamotrack from your checkout. Nothing was changed. You can [move the installation onto the release files](/configuration/upgrading#if-you-installed-with-git), or keep building from Git by adding these two lines at the top of `.env`:
...
<Accordion title="POSTGRES_PASSWORD error from Compose">
  ...
  Then open `.env`, replace `change-me` with a real password, and run `docker compose up -d --no-build --wait`.
</Accordion>
