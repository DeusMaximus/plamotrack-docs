...
<Accordion title="error from registry: denied, after updating">
  Your `.env` is older than the current Compose files and has no `COMPOSE_FILE` line, so Docker Compose looked for a published image instead of building plamotrack from your checkout. Nothing was changed. Add these two lines at the top of `.env`:
