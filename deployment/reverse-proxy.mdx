---
title: "Use plamotrack behind nginx, Traefik, or any reverse proxy"
...
Your proxy must do all five of these things:
...
<Steps>
  ...
  <Step title="Mind the proxy's access log">
    A proxy that logs full request URIs — most do by default — logs the one-time codes an OIDC callback carries. Turn that off, or trim the log to paths without their query strings, as the bundled web server does. See [Access logs and callback credentials](/configuration/audit-log#access-logs-and-callback-credentials).
  </Step>
</Steps>
