...
<Steps>
  ...
  <Step title="Hook the token into the Caddy service">
    ...
    The two lines hand Caddy the token from that root-only file. They're the repository's [`deploy/caddy/caddy.service.d/cloudflare.conf`](https://github.com/DeusMaximus/plamotrack/blob/main/deploy/caddy/caddy.service.d/cloudflare.conf) without its comments.
  </Step>
</Steps>

## Configure Caddy

Replace the contents of `/etc/caddy/Caddyfile` (`sudoedit /etc/caddy/Caddyfile`) with this, using your own hostname. It's the repository's [`deploy/caddy/Caddyfile`](https://github.com/DeusMaximus/plamotrack/blob/main/deploy/caddy/Caddyfile) without its comments:
...
There is no `log` directive — and that's intentional. A proxy access log records full request URIs, and in OIDC mode those include the one-time authorization codes from your identity provider's callbacks. Don't add one unless you have configured it to leave those out; see [Access logs and callback credentials](/configuration/audit-log#access-logs-and-callback-credentials).
...
You should get back `{"status":"ok"}`. `https://plamotrack.example/api/readyz` answers `404` — readiness is for the container's own healthcheck, so a stranger cannot learn whether the database is up. Then open the app in your browser and claim the instance — run `docker compose logs api | grep -A6 "no owner yet"` to find the one-time setup token.
